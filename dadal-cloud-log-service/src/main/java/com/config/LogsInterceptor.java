...
public class LogsInterceptor implements HandlerInterceptor {
	...
	/** 通过spring自动注入 */
	public void setServiceImpl(LogService service) {
	 ...
	 * 要在Interceptor之后调用的内容都写在调用invoke方法之后。 该方法在目标方法调用之后，渲染视图之前被调用；
	 * 可以对请求域中的属性或视图做出修改
	 */
	@Override
		...
		try {
			if (handler instanceof HandlerMethod) {
				...
				if (login_id != null) {
					...
					// 操作IP和时间记录
					//appLog.setIpAddress(getIp(request));
					//appLog.setCreateTime(date);
					// 获取访问方法的注解
					SysLog sysLog = methodHandler.getMethod().getAnnotation(SysLog.class);
					if (sysLog != null) {
						//appLog.setOpModule(sysLog.operationModule());
						//appLog.setIntroduce(sysLog.describe());
